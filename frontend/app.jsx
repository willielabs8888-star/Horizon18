    ...
    const LABEL_MAP = {
      ...
      computer_science: "Computer Science", engineering: "Engineering (General)",
      biology: "Biology / Pre-Med", environmental_science: "Environmental Science",
      mathematics: "Mathematics / Statistics", physics: "Physics",
      chemistry: "Chemistry", data_science: "Data Science",
      ...
      business_finance: "Business / Finance", accounting: "Accounting",
    ...
    function QuizPage({ onComplete }) {
      ...
      const renderSalaryStep = () => {
        ...
        return (
        <div className="quiz-step">
          <h2>Review estimated salaries</h2>
          <p className="hint">These are estimated starting salaries for {metroLabel} based on national data and local cost of living. Adjust any value if you have better information.</p>

          {salaryLoading && <div style={{textAlign:"center",padding:20,color:"var(--text-dim)"}}>Loading salary estimates...</div>}

          {!salaryLoading && sorted.map(inst => {
            const color = instanceColor(inst, instances);
            const pt = inst.path_type;

            return (
              <div key={inst.instance_id} className="card" style={{marginBottom: 12, borderLeft: `3px solid ${color}`, padding: "14px 16px"}}>
                <strong style={{color, marginBottom: 8, display:"block"}}>{instanceLabel(inst, instances)}</strong>

                {/* COLLEGE / CC_TRANSFER */}
                {(pt === "college" || pt === "cc_transfer") && (() => {
                  ...
                  return (
                    <div>
                      <div className="form-group" style={{marginBottom: 8}}>
                        <label style={{fontSize: 13}}>Estimated starting salary after graduation</label>
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <div style={{position:"relative",flex:1}}>
                            <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#666",fontSize:14}}>$</span>
                            <input type="number" className="form-select" style={{paddingLeft:22}} min="0" step="1000"
                              value={current || ""}
                        ...
                        <p className="field-hint" style={{marginTop:4}}>
                          Based on {(LABEL_MAP[majorKey] || majorKey || "your major").replace("_"," ")} graduates in {metroLabel}.
                          {pt === "cc_transfer" && " Includes a 2% discount for community college transfer."}
                          {salaryDefaults?.salary_growth?.[majorKey] != null &&
                            ` Grows at ${(salaryDefaults.salary_growth[majorKey] * 100).toFixed(1)}%/year above inflation.`}
                        </p>
                      </div>
                    </div>
                  );
                  ...
                  return (
                    <div>
                      <div className="form-group" style={{marginBottom: 10}}>
                        <label style={{fontSize: 13}}>Journeyman salary (after apprenticeship)</label>
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <div style={{position:"relative",flex:1}}>
                            <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#666",fontSize:14}}>$</span>
                            <input type="number" className="form-select" style={{paddingLeft:22}} min="0" step="1000"
                      ...
                      <div className="form-group" style={{marginBottom: 8}}>
                        <label style={{fontSize: 13}}>Apprentice wages (years 1–4)</label>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6}}>
                          {[0,1,2,3].map(yr => (
                            <div key={yr} style={{position:"relative"}}>
                              <span style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",color:"#888",fontSize:11}}>Yr {yr+1} $</span>
                              <input type="number" className="form-select" style={{paddingLeft:52,fontSize:13}} min="0" step="500"
                        ...
                        {aOverridden && (
                          <button className="btn btn-secondary" style={{fontSize:11,padding:"4px 6px",marginTop:4}}
                            onClick={() => updateInstance(inst.instance_id, "apprentice_wages_override", null)}>
                            Reset apprentice wages
                          </button>
                        )}
                        <p className="field-hint" style={{marginTop:4}}>
                          {(LABEL_MAP[tradeKey] || tradeKey || "").replace("_"," ")} wages progress from ~40% to ~85% of journeyman pay.
                          After journeyman, grows at 0.5%/year above inflation.
                        </p>
                      </div>
                    </div>
                  );
                ...
                {pt === "workforce" && (() => {
                  ...
                  return (
                    <div>
                      <div className="form-group" style={{marginBottom: 8}}>
                        <label style={{fontSize: 13}}>Starting annual wage</label>
                        <div style={{display:"flex",gap:8,alignItems:"center"}}>
                          <div style={{position:"relative",flex:1}}>
                            <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#666",fontSize:14}}>$</span>
                            <input type="number" className="form-select" style={{paddingLeft:22}} min="0" step="500"
                        ...
                        <p className="field-hint" style={{marginTop:4}}>
                          Based on {(LABEL_MAP[indKey] || indKey || "").replace("_"," ")} entry-level in {metroLabel}.
                          Grows at 0.5%/year above inflation.
                        </p>
                      </div>
                    </div>
                  );
                ...
                {pt === "military" && (() => {
                  ...
                  return (
                    <div>
                      <div className="form-group" style={{marginBottom: 10}}>
                        <label style={{fontSize: 13}}>Active duty compensation (federal — not adjustable)</label>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
                          {activeComp.map((c, i) => (
                            <div key={i} style={{fontSize:12,color:"var(--text-dim)",padding:"4px 8px",background:"var(--card-bg)",borderRadius:4}}>
                              Year {i+1}: {fmtFull(c)}
                            </div>
                          ))}
                        </div>
                      </div>
                      {inst.use_gi_bill && inst.gi_bill_major && (
                        <div className="form-group" style={{marginBottom: 8}}>
                          <label style={{fontSize: 13}}>Post-degree starting salary ({(LABEL_MAP[inst.gi_bill_major] || inst.gi_bill_major).replace("_"," ")})</label>
                          <div style={{display:"flex",gap:8,alignItems:"center"}}>
                            <div style={{position:"relative",flex:1}}>
                              <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#666",fontSize:14}}>$</span>
                              <input type="number" className="form-select" style={{paddingLeft:22}} min="0" step="1000"
                          ...
                          <p className="field-hint" style={{marginTop:4}}>Salary after using GI Bill to complete a degree in {metroLabel}.</p>
                        </div>
                      )}
                      {!inst.use_gi_bill && inst.civilian_industry && (
                        <div className="form-group" style={{marginBottom: 8}}>
                          <label style={{fontSize: 13}}>Post-service civilian wage ({(LABEL_MAP[inst.civilian_industry] || inst.civilian_industry).replace("_"," ")})</label>
                          <div style={{display:"flex",gap:8,alignItems:"center"}}>
                            <div style={{position:"relative",flex:1}}>
                              <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:"#666",fontSize:14}}>$</span>
                              <input type="number" className="form-select" style={{paddingLeft:22}} min="0" step="500"
                          ...
                          <p className="field-hint" style={{marginTop:4}}>Includes a 10% veteran hiring premium over standard {(LABEL_MAP[inst.civilian_industry] || "").replace("_"," ")} wages.</p>
                        </div>
                      )}
                    </div>
                  );
      ...
      return (
        <div>
            ...
            {showAdvanced && (
              <div style={{padding: "0 20px 20px", borderTop: "1px solid var(--border)"}}>
                ...
                <div style={{marginBottom: 20}}>
                  ...
                  <input type="range" min={0} max={50} step={1} value={Math.round(savingsRate * 100)}
                    onChange={e => { const v = parseInt(e.target.value) / 100; handleAssumptionChange(setSavingsRate, v); }}
                    style={{width: "100%", accentColor: "var(--accent)", cursor: "pointer"}} />
                  <p style={{fontSize: 11, color: "var(--text-dim)", marginTop: 4}}>
                    Percent of after-tax income saved annually. Most people save 5–15%.
                  </p>
                </div>
                ...
                <div style={{marginBottom: 20}}>
                  ...
                  <input type="range" min={0} max={200} step={5} value={Math.round(investReturn * 1000)}
                    onChange={e => { const v = parseInt(e.target.value) / 1000; handleAssumptionChange(setInvestReturn, v); }}
                    style={{width: "100%", accentColor: "var(--accent)", cursor: "pointer"}} />
                  <p style={{fontSize: 11, color: "var(--text-dim)", marginTop: 4}}>
                    Default: 6%/year (real, after inflation). Conservative: 4%. Aggressive: 8%.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <input type="range" min={10} max={40} step={1} value={Math.round(taxRate * 100)}
                    onChange={e => { const v = parseInt(e.target.value) / 100; handleAssumptionChange(setTaxRate, v); }}
                    style={{width: "100%", accentColor: "var(--accent)", cursor: "pointer"}} />
                  <p style={{fontSize: 11, color: "var(--text-dim)", marginTop: 4}}>
          ...
          <div className="disclaimer-banner">
            ...
            <span>All figures are in <strong>today's dollars</strong> (adjusted for inflation). A dollar shown at age 40 has the same purchasing power as a dollar today. These projections use simplified assumptions and national averages. Your actual results will vary.</span>
          </div>
            ...
            <div style={{lineHeight: 1.8, fontSize: 14}}>
              ...
              {sorted.length >= 2 && (() => {
                const byNW = [...sorted].sort((a, b) =>
                  b.snapshots[b.snapshots.length-1].net_worth - a.snapshots[a.snapshots.length-1].net_worth
                );
                ...
                return (
                  <div>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{marginBottom: 12}}>
                          <strong style={{display:"block", marginBottom: 6}}>Debt Overview</strong>
                          <table style={{width:"100%", borderCollapse:"collapse", fontSize:13}}>
                            <thead>
                              <tr style={{borderBottom:"2px solid var(--border)"}}>
                                <th style={{textAlign:"left", padding:"6px 8px"}}>Path</th>
                                <th style={{textAlign:"left", padding:"6px 8px"}}>Education Cost</th>
                                <th style={{textAlign:"left", padding:"6px 8px"}}>Debt-Free Age</th>
                                <th style={{textAlign:"left", padding:"6px 8px"}}>Peak Debt Burden</th>
                              </tr>
                            </thead>
                            <tbody>
                              {sorted.map(r => {
                                ...
                                return (
                                  <tr key={rid} style={{borderBottom:"1px solid var(--border)"}}>
                                    <td style={{padding:"6px 8px", color: colorMap[rid], fontWeight:600}}>{labelMap[rid]}</td>
                                    <td style={{padding:"6px 8px"}}>{edCost > 0 ? fmtFull(edCost) : "None"}</td>
                                    <td style={{padding:"6px 8px"}}>{debtFreeAge ? `Age ${debtFreeAge}` : edCost > 0 ? "Not within projection" : "No debt"}</td>
                                    <td style={{padding:"6px 8px"}}>{burden > 0 ? `${burdenPct}% (${level})` : "—"}</td>
                                  </tr>
                                );
                              })}
                            </tbody>
                          </table>
                          <span style={{color:"var(--text-dim)", fontSize:11, display:"block", marginTop:4}}>Peak debt burden = peak annual loan payment as % of take-home pay. Under 10% is comfortable; over 15% can be a strain.</span>
                        </div>
                      );
                    })()}
                        ...
                        {activeResult && (() => {
                          ...
                          return (
                            <div style={{padding: "14px 20px", borderTop: "1px solid var(--border)", background: "var(--bg)", fontSize: 12, color: "var(--text-dim)", lineHeight: 1.7}}>
                              ...
                              <p style={{marginBottom: 8}}><strong style={{color: "var(--text)"}}>Savings:</strong> Your savings target is your savings rate (e.g. 25%) applied to your take-home pay. If there isn't enough left after expenses and loan payments, you save what you can — so your actual savings rate may be lower than the target. During school, part-time income goes toward tuition first.</p>
            ...
            <div style={{marginTop: 20}}>
              ...
              <HiwDropdown icon="💼" title="Direct Workforce" badge="Start immediately">
                ...
                <div className="hiw-timeline">
                  ...
                  <div className="hiw-timeline-step"><strong>Every year after:</strong> Your salary grows at ~0.5% per year in real terms (above inflation). This is slower growth than paths requiring a degree, but you have a head start.</div>
                </div>
